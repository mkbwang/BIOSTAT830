...
\begin{table}[htbp]
		...
		\begin{tabular}{cccc}
			...
			\bottomrule
		\end{tabular}
		\caption{The effect sizes related to fully vaccination rates  of Translational Poisson log linear regression. The first column corresponds to model \ref{formulap3b} without any lags. The second column corresponds to model \ref{formulap3c} with two lags. Coefficients with $\star$ indicates that their P values are smaller than 0.05. There is no estimate for $\delta_2$ because of singularity issues.}\label{P3bc}
\end{table}

\item I can fit multiple models with different number of lags and compare the AIC of each Poisson regression model. Models with the lowest AIC is my preferred model.
\end{enumerate}
